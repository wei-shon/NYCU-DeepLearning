...
private:
	...
	struct lookup {
		...
		void init(int r) {
			raw = r; // E.g. : raw = 0xfdecba9876543210
			...
			//row 原本的樣子
			score = mvleft(L);
			left = ((L[0] << 0) | (L[1] << 4) | (L[2] << 8) | (L[3] << 12)); // left = 0xfdecba9876543210

			score = mvleft(R); std::reverse(R, R + 4);
			right = ((R[0] << 0) | (R[1] << 4) | (R[2] << 8) | (R[3] << 12));// right = 0x32107654ba98fdec
		}
...
public:
	...
	// void popup2(int position){
	// 	set(position, 1);
	// }
	// void popup4(int position){
	// 	set(position, 2);
	// }
...
public:
	...
	state select_best_move(const board& b) const {
		...
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) {
				...
				expact_value = total_value/num;
				move->set_value(move->reward()+expact_value);
	 ...
	 */
	void update_episode(std::vector<state>& path,std::vector<board> befor_action ,float alpha = 0.1) const {
		// TODO
		// state& new_state = path.back();
		// float target = estimate(befor_action.back());
		// board after = befor_action.back();
		float target = 0;
		for(path.pop_back() , befor_action.pop_back(); path.size(); path.pop_back(),befor_action.pop_back() ){
			// board before = befor_action.back();
			// state& previous_state = path.back();
			// float error = previous_state.reward() + estimate(after) -  estimate(before);
			// float a = update(before, alpha*error );
			// after = before;
...
int main(int argc, const char* argv[]) {
	...
	// tdl.load("./models/weight_load_score.bin");

	// train the model
	std::vector<state> path;
	std::vector<board> befor_action;
	std::vector<float> learning_score;
	path.reserve(20000);
	for (size_t n = 1; n <= total; n++) {
		...
		// update by TD(0)
		tdl.update_episode(path, befor_action , alpha);
		learning_score.push_back(score);
	...
	tdl.save("./models/weight_load_score.bin");
