...
class gaussian_lstm(nn.Module):
    ...
    def reparameterize(self, mu, logvar):
        #從 N(0,1) 採樣，再乘上 exp(log) 加上 mu ，讓他成為從 N(mu , log) 採樣出來的樣本
        # tensor_z = torch.normal(0, 1, size=(self.batch_size, self.output_size)).to('cuda')
        # standard = torch.exp(logvar)
        # tensor_z = tensor_z*standard + mu
        # return tensor_z
