...
def train(x, cond, modules, optimizer, kl_anneal, args):
    ...
    for i in range(1, args.n_past + args.n_future):
        ...
        ht  = h_seq[i][0]
        z_t, mu, logvar = modules['posterior'](ht)
        g_t = modules['frame_predictor'](torch.cat((cond[i-1] , ht_1 , z_t) , 1))
        pred = modules['decoder']( [g_t , skip] )
        # calculate mse
        mse += loss_fn(pred , x[i])
...
class kl_annealing():
    def __init__(self, args):
        super().__init__()
        self.beta = 0
        self.KL_period = np.ones(args.niter) *1.0
        self.NowIsWhichPeriod = 0   
        # raise NotImplementedError        
        if args.kl_anneal_cyclical:
            period = args.niter / args.kl_anneal_cycle
            step = 1 / (self.period * 0.5)
            for cycle in range(args.kl_anneal_cycle):
                KL_period_value = 0.0
                KL_period_index = 0
                while(KL_period_value <= 1.0 and int (KL_period_index + cycle*period) < args.niter):
                    self.KL_period[int (KL_period_index + cycle*period)] = KL_period_value
                    KL_period_value+=step
                    KL_period_index+=1
        else :
            self.period = args.niter
            step = 1 / (self.period * 0.25)
            KL_period_value = 0.0
            KL_period_index = 0
            while(KL_period_value <= 1.0 and KL_period_index < args.niter):
                self.KL_period[KL_period_index] = KL_period_value
                KL_period_value+=step
                KL_period_index+=1
            
    
    def update(self):
        self.NowIsWhichPeriod+=1
        # raise NotImplementedError
    
    def get_beta(self):
        return self.KL_period[self.NowIsWhichPeriod]
        # raise NotImplementedError
...
def main():
    ...
    if test_mode == False:
        ...
        for epoch in range(start_epoch, start_epoch + niter):
            ...
            train_pbar = tqdm(train_loader, position=0, leave=True, ascii = ' =')
            for _, (seq, cond) in enumerate(train_pbar):
                seq = seq.to(device)
                cond = cond.to(device)
            ...
            if epoch >= args.tfr_start_decay_epoch:
                ...
                decay = (1.0 - args.tfr_lower_bound) / (args.niter - args.tfr_start_decay_epoch)
                tfr = 1.0 - (epoch - args.tfr_start_decay_epoch) * decay
                args.tfr = min(1, max(args.tfr_lower_bound, tfr))

                # args.tfr = args.tfr * args.tfr_decay_step
                # raise NotImplementedError
            print('teacher forcing : ' , args.tfr )
        ...
        for _ in range(len(test_data) // args.batch_size):
            ...
            test_cond = test_cond.permute(1,0,2)
        ...
        ave_psnr = np.mean(np.concatenate(psnr_list))
        print()
        print(('====================== test psnr = {:.5f} ========================'.format(ave_psnr)))
